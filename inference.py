...
def infer(model, device):
    ...
    output_folder = "output/MSCMR_exp/"
    model_path = "output/MSCMR_exp/checkpoint0099.pth"
...
@torch.no_grad()
def evaluate(model, criterion, dataloader_dict, device, output_dir, visualizer, epoch, writer):
    model.eval()
    criterion.eval()
    ...
    for samples, targets in dataloader:
        ...
        if step == 8:
            ...
            _, pre_masks = torch.max(outputs[0], 0, keepdims=True)
...
def infer_folder(model, device):
    ...
    model_name_list = [name for name in os.listdir(output_folder) if name.split('.')[-1] == 'pth']

    for name in model_name_list:
        model_path = os.path.join(output_folder, name)
        print()
        print(model_path)
        if not os.path.exists(output_folder):
            os.makedirs(output_folder)
