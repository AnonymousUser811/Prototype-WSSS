...
def get_args_parser():
    ...
    parser.add_argument('--lr_drop', default=700, type=int)
    ...
    parser.add_argument('--model', default='Unet_p', type=str) # Unet, Unet_p, ViGUNet
    parser.add_argument('--output_dir', default='output/MSCMR_exp/', help='path where to save, empty for no saving')
    parser.add_argument('--save_threshold', default=0.78, type=float)
...
def main(args):
    ...
    losses = ['CrossEntropy']
    model, criterion, visualizer = build_model(args, losses)
    model.to(device)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        _, criterion, _ = build_model(args, losses)

        test_stats = evaluate(
            model, criterion, dataloader_val_dict, device, args.output_dir, visualizer, epoch, writer
        )
        ...
        if args.output_dir:
            ...
            if dice_score > args.save_threshold:
